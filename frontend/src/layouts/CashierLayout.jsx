...
import { Banknote, LayoutDashboard, Receipt, FileText, History, LogOut, Menu, X, User as UserIcon, Search } from 'lucide-react';
...
const CashierLayout = () => {
    ...
    const menuItems = useMemo(() => {
        ...
        return filterMenuByPermission(user, [
            { label: 'Dashboard', path: '/cashier', icon: LayoutDashboard, permission: 'cashier.dashboard.view', keywords: ['home', 'main', 'index', 'overview', 'summary'] },
            ...
            { label: 'Record Payment', path: '/cashier/payments/new', icon: Receipt, permission: 'cashier.payments.create', keywords: ['pay', 'new', 'payment', 'record'] },
            { label: 'Payment History', path: '/cashier/payments', icon: History, permission: 'cashier.payments.view', keywords: ['history', 'log', 'payments', 'past'] },
            { label: 'Salary Payments', path: '/cashier/salary-payments', icon: Banknote, permission: 'payroll.pay', keywords: ['salary', 'payroll', 'employee', 'staff payout'] },
            { label: 'My Profile', path: '/cashier/profile', icon: UserIcon, keywords: ['profile', 'account', 'personal', 'avatar'] }
        ]);
    }, [user]);
        ...
        <div className="phoenix-app-shell">
            <header className="phoenix-global-topbar">
                <div className="phoenix-topbar-brand">
                    ...
                    <Link to="/cashier" className="flex min-w-0 items-center">
                ...
                <div className="phoenix-topbar-content">
                    ...
                    <div className="ml-auto flex items-center gap-3">
                        ...
                        <Link to="/cashier/profile" aria-label="Open my profile"><UserAvatar user={user} /></Link>
                    </div>
                </div>
            </header>
            ...
            >
                <div className="flex h-full flex-col">
                    <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4">
                        <p className="school-sidebar-label px-3 pb-2 pt-1">Cashier Menu</p>
                        {menuItems.map((item) => {
                            const isActive = item.path === '/cashier' || item.path === '/cashier/payments' || item.path === '/cashier/payments/new'
